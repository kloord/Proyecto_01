...
// Función para mostrar la matriz 2D gráficamente
void mostrarMatrizGraficamente(const vector<vector<double>>& matriz) {
    cout << "Gráfico de la matriz 2D:" << endl;
    for (size_t i = 0; i < matriz.size(); ++i) {
        for (size_t j = 0; j < matriz[i].size(); ++j) {
            int num_barras = static_cast<int>(matriz[i][j]); // Convertir el valor a entero
            // Imprimir caracteres de barras correspondientes al valor
            for (int k = 0; k < num_barras; ++k) {
                cout << "#";
            }
            cout << "  "; // Espacio entre columnas
        }
        cout << endl; // Nueva línea para la siguiente fila
    }
}
...
int main() {
   ...
   if (cargo == 1) {
          ...
          mostrarMatrizGraficamente(monto);
      }
